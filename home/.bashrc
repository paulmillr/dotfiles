...
if [[ $- == *i* ]]; then
  ...
  _bash_prompt_update() {
    ...
    # Build the prompt after collecting Git data so branch names remain plain
    # text. With promptvars disabled, Bash will not recursively evaluate any
    # command substitutions that a branch name happens to contain.
    PS1='\[\033[94m\]\W\[\033[0m\]'
    PS1+="${_bash_prompt_vcs_info} "
    PS1+='\['
    PS1+="$_bash_prompt_symbol_color"
    PS1+='\]'
    PS1+="$_bash_prompt_symbol"
    PS1+='\[\033[0m\] '
  }
