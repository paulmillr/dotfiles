...
function! s:highlightFromGroup(group) abort
    ...
    let l:highlight = ['cterm=NONE', 'ctermbg=NONE', 'gui=NONE', 'guibg=NONE']
    if !empty(l:ctermfg)
        call add(l:highlight, 'ctermfg=' . l:ctermfg)
    endif
    if !empty(l:guifg)
        call add(l:highlight, 'guifg=' . l:guifg)
    endif
    return join(l:highlight, ' ')
endfunction
